...
namespace keepr_c.Repositories
{
  public class KeepsRepository:DbContext
  {
    ...
    {
      int id=_db.ExecuteScalar<int>("INSERT INTO keeps (name,description,address,userId,privatepublic,views,keeps) VALUES(@Name,@Description,@Address,@UserId,@Private,0,0); SELECT LAST_INSERT_ID();", newKeep);
      newKeep.Id=id;
      return newKeep;
    }
